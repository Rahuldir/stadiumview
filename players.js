/* ══════════════════════════════════════════════════════════════
   StadiumView — Kinematics v19.0 (Rigid Ground & Safe FK Update)
   • FIXED: Quicksand/Clipping bug. A hard Y-axis clamp makes the floor 100% rigid.
   • FIXED: Keeper crouch and Sprint cycles recalibrated using Safe-Angle FK so legs bend UP, not DOWN.
   • MAINTAINS: Autonomous AI fielding, exact pitch targeting, and team liveries.
   ══════════════════════════════════════════════════════════════ */
(function(){
  'use strict';

  console.log('%c[players.js] IIFE started — kinematics v19.0 (Rigid Floor)', 'color:#00ff55;font-weight:bold');
  ...
  function boot(rawPlayers){
    ...
    function resetToRest(p){
      ...
      // THE RIGID FLOOR LOCK: Prevents the entire group from dipping below the original spawn height.
      p.group.position.y = p.home.y; 
    }

    // ═════════════════════════════════════════════════════════════
    //  SAFE-ANGLE KINEMATICS: Prevents stretching into the floor
    // ═════════════════════════════════════════════════════════════
    function torso(p, t){
      const sk = p.skel;
      if (p.role === 'Striker') {
        dBend(sk, 'lThigh', -0.2); dBend(sk, 'lShin', 0.3);
        dBend(sk, 'rThigh', -0.2); dBend(sk, 'rShin', 0.3);
        ...
        // Safe FK Crouch: Shallow leg bends paired with a forward torso lean. No quicksand clipping.
        dBend(sk, 'lThigh', -0.6); dBend(sk, 'lShin', 0.8); 
        dBend(sk, 'rThigh', -0.6); dBend(sk, 'rShin', 0.8); 
        dX(sk, 'spine', 0.3); 
    ...
    function runCycle(p){
      ...
      dX(sk, 'lForeArm', -0.6 - Math.sin(rc) * 0.3); 
      dX(sk, 'rForeArm', -0.6 + Math.sin(rc) * 0.3); 

      // Safe FK Sprint: Amplitude reduced to 0.6 so the feet do not extend past their resting length
      const swing = Math.sin(rc) * 0.6;
      dBend(sk, 'lThigh', -swing);
      dBend(sk, 'rThigh', swing);
      
      // Knee bends UPWARDS to follow the lift, never down into the floor
      dBend(sk, 'lShin', Math.max(0, -swing * 1.2));
      dBend(sk, 'rShin', Math.max(0, swing * 1.2));
    }
    ...
    console.log('[PlayerControl] ✅ Ready — Rigid Floor & Safe FK Active');
  }
})();
